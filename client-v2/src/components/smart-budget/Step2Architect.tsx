  ...
  return (
    ...
    >
      ...
      <div className="flex-none bg-card border-b border-border px-6 py-4 flex items-center justify-between z-10">
        ...
        {/* Income Summary Pill */}
        <div className="hidden md:flex items-center gap-6 bg-muted px-6 py-2 rounded-full border border-border">
          <div className="text-right">
            <span className="block text-[10px] font-medium text-muted-foreground uppercase tracking-wide">Total Income</span>
            <span className="block text-lg font-display font-bold text-foreground">kr {formatNumber(state.income)}</span>
          </div>
          <div className="h-8 w-px bg-border"></div>
          <div className="text-left">
            <span className="block text-[10px] font-medium text-muted-foreground uppercase tracking-wide">Unallocated</span>
            <span className={cn(
              "block text-lg font-display font-bold",
              unallocated < 0 ? "text-[oklch(var(--theme-coral))]" : "text-[oklch(var(--theme-teal))]"
      ...
      {state.income > 0 && (
        <div className="flex-none bg-card border-b border-border px-6 py-2 text-[11px] text-muted-foreground">
          <span
            className={cn(
              "font-medium",
              needsActualPercent > needsTargetPercent + 5 ? "text-[oklch(var(--theme-coral))]" : "text-foreground"
            )}
          >
      ...
      {/* Content */}
      <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-20 bg-background">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">

          {/* Left Column: Fixed Expenses */}
          <div className="lg:col-span-4 space-y-4">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Fixed Expenses</h2>
              <span className="text-xs font-medium text-muted-foreground">Total: kr {formatNumber(totalFixed)}</span>
            </div>
            ...
            <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
              <div className="divide-y divide-border/50">
                {fixedCats.map(category => {
                  ...
                  return (
                    <div key={category.id} className="p-4 hover:bg-muted transition-colors group">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          ...
                          {isBill && (
                            <span className="ml-1 text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground uppercase tracking-wide">
                              Bill
                            </span>
                          )}
                        </div>
                      ...
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-xs font-bold">kr</span>
                        <input
                          type="text"
                          value={amount > 0 ? formatNumber(amount) : ''}
                          placeholder="0"
                          onChange={(e) => handleFixedChange(category.id, e.target.value)}
                          className="w-full pl-8 pr-3 py-2 bg-muted border-0 rounded-lg focus:ring-2 focus:ring-[oklch(var(--theme-teal)/0.2)] focus:bg-card transition-all font-bold text-foreground text-right placeholder:text-muted-foreground/50"
                        />
                      </div>
                    </div>
                  );
                ...
                <button className="w-full py-3 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors flex items-center justify-center gap-2">
          ...
          <div className="lg:col-span-8 space-y-4 flex flex-col">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Variable Lifestyle & Savings</h2>
              <span className="text-xs font-medium text-muted-foreground">Disposable: kr {formatNumber(disposableIncome)}</span>
            </div>

            <div className="bg-card rounded-2xl shadow-sm border border-border flex-1 p-6 space-y-6 min-h-[400px]">
              ...
              {variableCats.map(category => {
                ...
                return (
                  <div
                    key={category.id}
                    className={cn(
                      "bg-card rounded-xl border p-4 shadow-sm hover:shadow-md transition-all duration-300",
                      isSavings ? "border-[oklch(var(--theme-teal)/0.3)] ring-1 ring-[oklch(var(--theme-teal)/0.1)]" : "border-border"
                    )}
                  >
                      ...
                      <div className="flex items-center gap-2">
                        <div className="text-right relative">
                          <input
                            ...
                            className={cn(
                              "w-28 text-right font-bold text-lg border-b focus:border-[oklch(var(--theme-teal))] outline-none bg-transparent transition-colors pb-1",
                              isSavings ? "text-[oklch(var(--theme-teal))] border-[oklch(var(--theme-teal)/0.3)]" : "text-foreground border-border"
                            )}
                          />
                          <span className="text-xs text-muted-foreground ml-1 absolute right-0 -bottom-5">kr</span>
                        </div>
                      </div>
                    </div>
                    <input
                      ...
                      className={cn(
                        ...
                        "accent-foreground" // default fallback
                      )}
                    ...
                    <div className="flex justify-between mt-2">
                      ...
                      <span className={cn(
                        "text-[10px] font-bold",
                        isSavings ? "text-[oklch(var(--theme-teal))]" : "text-foreground"
            ...
            <Button
              onClick={onSave}
              variant="teal"
              className="w-full mt-2 py-4 h-auto rounded-xl font-semibold shadow-xl shadow-[oklch(var(--theme-teal)/0.15)] hover:shadow-[oklch(var(--theme-teal)/0.25)] hover:translate-y-[-2px] transition-all flex items-center justify-center gap-2 text-lg"
            >
